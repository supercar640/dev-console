...
> **갱신(2026-05-30):** 위 stdin/stdout **직접 파싱 대신 공식 Agent SDK(`@anthropic-ai/claude-agent-sdk`)** 를 `ClaudeCodeAdapter`의 구현 토대로 채택. 직접 파싱은 권한 요청(`can_use_tool`)을 받지 못함(버그 #34046, M3 게이트 PoC에서 재현). SDK는 동일 프로토콜을 래핑하고 `canUseTool` 콜백으로 권한/질문 승인 흐름을 제공한다. 근거·증명: `dev-console-direction.md` §2-bis.
...
**채널은 UI 탭으로 전환**한다. 두 채널 동시 실행은 같은 폴더 파일 충돌 위험이 있으나, **금지하지 않고 허용**한다 — 예: Agent로 Claude 작업 중 Terminal에서 Codex 코드리뷰. 탭 전환·동시 시작 시 **확인창은 띄우지 않는다**(터미널을 여는 사용자는 충돌 위험을 알고 여는 것으로 보고, 위험은 사용자가 감수). (소유자 결정 이력: 동시 활성 금지 → 경고 후 허용(2026-05-31) → 경고 제거(2026-06-04). 상세: `dev-console-m3-ui-design.md`.)
