...
**채널은 UI 탭으로 전환**한다. 두 채널 동시 실행은 같은 폴더 파일 충돌 위험이 있으나, **금지하지 않고 "경고 후 허용"** 한다 — 예: Agent로 Claude 작업 중 Terminal에서 Codex 코드리뷰. 한 채널 실행 중 다른 채널을 시작하면 파일충돌 경고 confirm을 띄우고, 사용자가 확인하면 둘 다 실행한다. (소유자 결정 2026-05-31. 이전엔 "동시 활성 금지"였음 — 상세: `dev-console-m3-ui-design.md`.)
