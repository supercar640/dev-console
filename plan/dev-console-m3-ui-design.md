...
3. **듀얼채널 = 경고 후 동시 허용.** ⚠️ **명세 §2-2의 "동시 활성 금지"를 조정한다.** 근거(소유자 결정): Claude를 Agent로 돌리면서 Terminal에서 Codex로 코드리뷰하는 등 합당한 동시 사용이 있다. 막지 않고, 한 채널 실행 중 다른 채널 **시작 시 파일충돌 경고 → 확인하면 둘 다 실행**. 파일충돌 위험은 사용자가 감수.
...
- `views/Workspace.tsx` *(생성)* — 채널 탭 + 헤더(← 대시보드). `channel: 'agent'|'terminal'` 상태. AgentView/Terminal 중 하나 렌더. **듀얼채널 가드**: 비활성 채널 "시작" 시 다른 채널이 실행 중이면 confirm 경고.
...
- 듀얼채널 동시 실행은 **허용**하므로 충돌 방지 코드는 없음 — 경고 confirm만(사용자 책임).
...
- **수동 스모크**: `pnpm dev` → 프로젝트 열기 → 에이전트 시작 → "파일 써줘" → 인라인 승인 카드 + 윈도우 토스트 + 트레이 배지 확인 → 거부 → 채널 탭 전환 + 터미널 동시 시작 경고 확인.
...
- **명세 §2-2 갱신 필요**: "동시 활성 금지" → "경고 후 허용". 이 설계 승인 후 spec.md에 반영.
