...
3. **사람이 보는 터미널 출력을 정규식으로 긁어 상태 추론 금지.** (AO가 15커밋 갈아넣고 폐기한 길)
...
원본: `C:\AI_project\testbed\agent-orchestrator` (MIT). 매핑은 `plan/dev-console-reuse-map.md`.
- **함수 단위 복붙 + 의존성 끊기.** 모노레포 패키지를 link하지 말 것 (의존성 거미줄).
- worktree/PR/이슈 결합 로직은 제거하고 골격만 차용.
- 차용 시 출처 주석 남길 것 (예: `// adapted from agent-orchestrator/packages/.../pty-host.ts`).
...
- OS: Windows 11. Node v24.14, npm 11.9, pnpm 11.2.2, git 2.52, Python 3.14.
...
- **로컬 머지 금지.** 통합은 `push` → PR → 원격(GitHub) 경유. 마스터가 여러 기기에서 작업하므로 원격이 단일 진실 공급원.
